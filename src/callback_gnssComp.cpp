...
namespace decodeNav {
    ...
    void GnssCompassCallback::decode_ID25(const std::vector<uint8_t>& packet, DataFrameID25& frame) {
        // Define static expected size
        constexpr size_t header_size_ = 5;      // 5-byte header (LRC, ID, Length, CRC)
        constexpr size_t data_size_ = 12;       // Packet data length is 12 bytes
        constexpr size_t expected_size_ = header_size_ + data_size_; // Total packet size is 17 bytes
        constexpr uint8_t packet_id_ = 0x19;    // Packet ID is 25 (0x19)
    ...
    void GnssCompassCallback::decode_ID26(const std::vector<uint8_t>& packet, DataFrameID26& frame) {
        ...
        constexpr size_t data_size_ = 12;       // Packet data length is 12 bytes
        constexpr size_t expected_size_ = header_size_ + data_size_; // Total packet size is 17 bytes
        constexpr uint8_t packet_id_ = 0x1A;    // Packet ID is 26 (0x1A)
    ...
    void GnssCompassCallback::decode_ID28(const std::vector<uint8_t>& packet, DataFrameID28& frame) {
        // Define static expected size
        constexpr size_t header_size_ = 5;      // 5-byte header (LRC, ID, Length, CRC)
        constexpr size_t data_size_ = 48;       // Packet data length is 48 bytes
        constexpr size_t expected_size_ = header_size_ + data_size_; // Total packet size is 53 bytes
        constexpr uint8_t packet_id_ = 0x1C;    // Packet ID is 28 (0x1C)
    ...
    void GnssCompassCallback::decode_ID29(const std::vector<uint8_t>& packet, DataFrameID29& frame) {
        // Define static expected size
        constexpr size_t header_size_ = 5;      // 5-byte header (LRC, ID, Length, CRC)
        constexpr size_t data_size_ = 74;       // Packet data length is 74 bytes
        constexpr size_t expected_size_ = header_size_ + data_size_; // Total packet size is 79 bytes
        constexpr uint8_t packet_id_ = 0x1D;    // Packet ID is 29 (0x1D)
